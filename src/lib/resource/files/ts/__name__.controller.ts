...
export class <%= classify(name) %>Controller {
  ...
  @Get()
  async findAll(@Query() query?: Record<string, any>): Promise<Array<<%= singular(classify(name))%>>> {
  ...
  @Get()
  async findOneQuery(@Query() query?: Record<string, any>): Promise<<%= singular(classify(name)) %>> {
  ...
  @Patch()
  async updateQuery(@Body() update<%= singular(classify(name)) %>: Update<%= singular(classify(name)) %>, @Query() query?: Record<string, any>): Promise<<%= singular(classify(name)) %>> {
  ...
  @Delete()
  async removeQuery(@Query() query?: Record<string, any>): Promise<<%= singular(classify(name)) %>> {
  ...
  async findAll(@Payload query?: Record<string, any>): Promise<Array<<%= singular(classify(name)) %>>> {
    return this.<%= lowercased(name) %>Service.findAll(query)
  }

  @MessagePattern(Topic.FIND_ONE_<%= singular(uppercased(name)) %>)
  async findOne(@Payload() id: string | Record<string, any>): Promise<<%= singular(classify(name)) %>> {
    return this.<%= lowercased(name) %>Service.findOne(id)
  }

  @MessagePattern(Topic.UPDATE_<%= singular(uppercased(name)) %>)
  async update(@Payload() id: string | Record<string, any>, @Payload() update<%= singular(classify(name)) %>: Update<%= singular(classify(name)) %>): Promise<<%= singular(classify(name)) %>> {
    return this.<%= lowercased(name) %>Service.update(id, update<%= singular(classify(name)) %>)
  }

  @MessagePattern(Topic.DELETE_<%= singular(uppercased(name)) %>')
  async remove(@Payload() id: string | Record<string, any>): Promise<<%= singular(classify(name)) %>> {
    return this.<%= lowercased(name) %>Service.remove(id)
  }<% } %>
}
